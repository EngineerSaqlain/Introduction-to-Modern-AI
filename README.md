**🤖 Introduction to Modern AI**




This repository documents my learning journey through the Cisco Networking Academy – Introduction to Modern AI course.

It contains notes, resources, examples, and practical insights related to modern Artificial Intelligence technologies.

The goal of this repository is to organize my learning, explore AI concepts, and share helpful resources for beginners interested in AI, chatbots, and prompt engineering.

**📚 Topics Covered**

🧠 Introduction to Artificial Intelligence

🤖 Modern AI tools and applications

💬 Chatbots and conversational AI

✍️ Prompt engineering basics

⚙️ AI models and how they work

⚖️ Ethical considerations in AI

🤖 Chatbots

This section explores chatbot technologies and conversational AI.

**Topics include:**

What chatbots are

How chatbots work

Real-world chatbot applications

Example chatbot prompts and responses

✍️ Prompt Writing

Prompt engineering is an essential skill when working with modern AI tools.

This repository includes:

How to write effective prompts

Best practices for prompt design

Examples of good vs bad prompts

Techniques to improve AI responses

📂 Learning Resources

Resources collected during the course:

📘 Course notes

📄 AI articles and documentation

🔗 Useful learning links

🛠 AI tools and platforms

🚀 Learning Objectives

Understand fundamental AI concepts

Learn how modern AI systems work

Explore chatbots and conversational AI

Practice effective prompt writing

Build a structured AI learning resource

📌 Repository Status

📈 This repository will be continuously updated as I progress through the Cisco Networking Academy AI course and learn new AI concepts.

**👨‍💻 Author**

Saqlain
🎓 Computer Systems Engineer – UET Peshawar
💡 Interested in Artificial Intelligence, Data Science, and Technology

*⭐ If you find this repository helpful, consider starring it.*
